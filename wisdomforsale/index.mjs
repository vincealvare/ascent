...
if(role === 'seller') {
    const sellerInteract = {
        ...
        wisdom: await ask.ask('Enter a wise phrase, or press Enter for default: ', (s) => {
            // w / s = wisdom / string shorthand?
            let w = !s ? 'Build healthy dApps with Reach.' : s;
            if (!s) {
                console.log(w);
            }
            return w;
    ...
    const buyerInteract = {
        ...commonInteract(role),
        confirmPurchase: async (price) => await ask.ask(`Do you want to purchase this wisdom for ${toSU(price)} ${suStr}?`, ask.yesno),
    ...
    await showBalance(acc);
    await ctc.participants.Buyer(buyerInteract);
    await showBalance(acc);
};
